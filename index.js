...
const emoteList = [
  "101", "418", "a", "ac", "amogus", "amogus2", "arrow", "atmg", "bacteria", "basic", "booster", "braindmg", "bushwhacker", "cap", "d2", "eight", "ent", "evaporate", "fighter", "fish", "five", "fiveandahalf", "four", "fourandahalf", "ft", "gigabrain", "half", "it", "lol", "n", "negativeinfinity", "nine", "nt", "one", "oneandahalf", "police", "police2", "ranger", "rlq", "seven", "six", "skull", "slt", "smt", "spectral", "spectral", "spin", "st", "sus", "t", "ten", "three", "threeandahalf", "troll", "trust", "trustnt", "two", "twoandahalf", "untrust", "vt", "vt2", "youtuber", "zero", "zlqi",
  ...
  "downarrow", "bread", "mythic", "whywouldyoudothat", "okfine", "hexabrid", "insane"
...
/servers - get data for the current arras.io servers\n
**Directors+ Only**\n
...
function navSubtab(parent, location) {
  return () => {
    ...
    [...document.querySelector("#specialSubtabsNav").children].forEach((child) => {
      child.disabled = false;
  ...
  message = message.replace(
    ...
    (gmatch, emoteId) => {
      ...
      const width = (
        ...
        emoteId === "veryuntrust" || emoteId === "mspt"
      ) ? 90 : 30;
...
function addOptPings(string, addPings) {
  ...
  if (string.includes("@" + elements.usernameInput.value) && Date.now() > pingerCooldown) {
    pinger.play();
  ...
  try {
    ...
    if (data.broadcast) {
      ...
      if (!data.imgURL) {
        ...
        outputHTML += "<br><img class=\"uploadImage\" src=\"" + data.imgURL + "\"><div class='uploadPad'></div>";
      } else if (data.imgMIME === "audio") {
        outputHTML += "<br><audio controls src=\"" + data.imgURL + "\">";
      }
      outputHTML += "<button class=\"replyButton\">Reply</button>";
    }
...
window.addEventListener("blur", () => gAudio.volume = 0);
